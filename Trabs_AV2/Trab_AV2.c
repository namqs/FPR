...
int limiteInsercao(TLista L, int numero);
...
int limiteInsercao(TLista L, int numero)
{
    ...
    //conta quantas vezes o numero ja aparece na lista
    while (aux != NULL)
    {
        if (aux->valor == numero)
        {
    ...
    //se o valor X já tiver aparecido X vezes, não será inserido mais
    if (contador >= limite)
    {
        printf("\nO número %d excedeu o limite de inserções (%d)!", numero, limite);
        return FALSE;
    }
